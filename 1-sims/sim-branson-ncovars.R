...
cat("numCovars, permP", file=paste0(resDir, "/sims/branson-ncovars-out.txt"),sep="\n", append=FALSE)
...
for (numCovarsX in c(1,2,3,4,5,10,20,30,40,50)) {
  ...
  # create data frame dfX with 1 covariate
  dfX = data.frame(V1 = rnorm(n, 0,1))
  
  # add more covariates to data frame dfX
  if (numCovarsX>1) {
    for (i in 2:numCovarsX) {
      
      dfX[,paste0("V",i)] = rnorm(n, 0,1)
  ...
  cat(paste0(numCovarsX,",",pvalue), file=paste0(resDir, "/sims/branson-ncovars-out.txt"),sep="\n", append=TRUE)
