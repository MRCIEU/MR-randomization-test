...
for a=1:length(all_hpEffect)
	...
	for e=1:length(all_rCovars)
		...
		% branson
		lower=allx.powerBransonNot(ix) - 1.96*allx.mcseBransonNot(ix);
		upper=allx.powerBransonNot(ix) + 1.96*allx.mcseBransonNot(ix);
		hold on; h1=plot([posx,posx], [lower, upper], '-', 'color', colorx{1}, 'linewidth', 3);
		hold on; h1=plot(posx, allx.powerBransonNot(ix), markersx{e}, 'MarkerFaceColor', facecolorx{1}, 'MarkerEdgeColor', colorx{1}, 'MarkerSize', markersizex);

		hxBran(e) = h1;

		% bonferroni
		posx = posx+0.02;
		lower=allx.powerBonNot(ix) - 1.96*allx.mcseBonNot(ix);
                upper=allx.powerBonNot(ix) + 1.96*allx.mcseBonNot(ix);
		hold on; h1=plot([posx,posx], [lower, upper], '-', 'color', colorx{2}, 'linewidth', 3);
                hold on; h1=plot(posx, allx.powerBonNot(ix), markersx{e}, 'MarkerFaceColor', facecolorx{2}, 'MarkerEdgeColor', colorx{2}, 'MarkerSize', markersizex);
		...
		% number of independent tests based on correlation
                posx = posx+0.02;
                lower=allx.powerIndLiNot(ix) - 1.96*allx.mcseIndLiNot(ix);
                upper=allx.powerIndLiNot(ix) + 1.96*allx.mcseIndLiNot(ix);
                hold on; h1=plot([posx,posx], [lower, upper], '-', 'color', colorx{4}, 'linewidth', 3);
                hold on; h1=plot(posx, allx.powerIndLiNot(ix), markersx{e}, 'MarkerFaceColor', facecolorx{4}, 'MarkerEdgeColor', colorx{4}, 'MarkerSize', markersizex);

                hxIndLi(e) = h1;

		% number of independent tests based on correlation
                posx = posx+0.03;
                lower=allx.powerRsqNot(ix) - 1.96*allx.mcseRsqNot(ix);
                upper=allx.powerRsqNot(ix) + 1.96*allx.mcseRsqNot(ix);
                hold on; h1=plot([posx,posx], [lower, upper], '-', 'color', colorx{5}, 'linewidth', 3);
                hold on; h1=plot(posx, allx.powerRsqNot(ix), markersx{e}, 'MarkerFaceColor', facecolorx{5}, 'MarkerEdgeColor', colorx{5}, 'MarkerSize', markersizex);
...
% save to file
filename=strcat(resDir, '/sims/hp/fig-sim-nothp-hpsnp',num2str(numSNPsHP),'-notHPsnp',num2str(numSNPsNOTHP),'-', num2str(hpEffect),'.pdf')
saveas(h, filename);
